...
const Dashboard = () => {
  const { user, profile, isAdmin, isConsultant, isLoading: isAuthLoading } = useAuth(); // Use context instead of manual fetch
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      // Try LocalStorage first (for instant feel)
      ...
      if (profile?.email) {
        ...
        if (remoteLeads && remoteLeads.length > 0) {
          ...
          setLastDiagnostic(remoteLeads[0]); // Most recent
        }
      }
    ...
    <Layout user={user}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
          <div>
            <h1 className="text-3xl font-bold text-white font-space">Hola, {profile?.name || user?.email || "Gastronómico"}</h1>
        ...
        {/* Client Project Link */}
        {profile?.role === 'client' && (
          <div className="mb-8 bg-gradient-to-r from-slate-900 to-slate-800 p-6 rounded-xl border border-slate-700 flex justify-between items-center shadow-lg">
            <div>
              <h2 className="text-xl font-bold text-white mb-1">Tu Proyecto en Curso</h2>
              <p className="text-slate-400">Accede al detalle de hitos, entregables y estado.</p>
            </div>
            <Link to="/hub/my-project">
              <Button className="bg-[#1FB6D5] text-[#021019] hover:bg-white font-bold px-6">
                Ver Mi Proyecto
              </Button>
            </Link>
          </div>

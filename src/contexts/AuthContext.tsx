...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchProfile = async (userId: string, email?: string, metadata?: Record<string, unknown>, retryCount = 0) => {
    ...
    try {
      ...
      if (userData) {
        // Cuenta rechazada → forzar signOut y avisar
        if (userData.rejected_at) {
          logger.warn('Cuenta rechazada, cerrando sesión', { context: 'Auth', data: { userId } });
          try {
            if (typeof window !== 'undefined') {
              alert('Tu cuenta fue rechazada. Si creés que es un error, contactá al administrador.');
            }
          } catch {}
          if (supabase) {

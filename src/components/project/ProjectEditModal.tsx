...
const ProjectEditModal: React.FC<ProjectEditModalProps> = ({ project, isOpen, onClose, onSave }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4 animate-fade-in backdrop-blur-sm">
            <div className="bg-[var(--bg-base)] rounded-md border border-[var(--border-subtle)] w-full max-w-2xl shadow-2xl flex flex-col max-h-[90vh]">
                <div className="flex justify-between items-center p-6 border-b border-[var(--border-subtle)]">
                    <h2 className="text-xl font-bold text-[var(--text-primary)]">Editar Proyecto</h2>
                ...
                <div className="p-6 overflow-y-auto space-y-8 custom-scrollbar">
                    ...
                    <div className="space-y-4">
                        <h3 className="text-sm font-bold text-[var(--color-primary)] uppercase border-b border-[var(--border-subtle)] pb-2">Información General</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div className="space-y-1">
                                <label className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider">Estado</label>
                                <select
                                    className="w-full bg-[var(--bg-base)] border border-[var(--border-subtle)] text-[var(--text-secondary)] text-sm rounded-lg p-2.5 outline-none"
                            ...
                            />
                            <div className="space-y-1">
                                <label className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider">Fase</label>
                                <select
                                    className="w-full bg-[var(--bg-base)] border border-[var(--border-subtle)] text-[var(--text-secondary)] text-sm rounded-lg p-2.5 outline-none"
                    ...
                    <div className="space-y-4">
                        <h3 className="text-sm font-bold text-[var(--color-primary)] uppercase border-b border-[var(--border-subtle)] pb-2">Equipo y Cliente</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <Input
                                    ...
                                    availableConsultants.map(c => (
                                        <button
                                            ...
                                            className={`flex items-center gap-2 p-2 rounded-lg border text-xs transition-all ${selectedConsultants.includes(c.id)
                                                ...
                                                : 'bg-[var(--bg-base)] border-[var(--border-subtle)] text-[var(--text-muted)] hover:border-[var(--border-subtle)]'
                                                }`}
                                        >
                                            <div className={`w-2 h-2 rounded-full ${selectedConsultants.includes(c.id) ? 'bg-[var(--color-primary)] shadow-[0_0_8px_rgba(34,211,238,0.5)]' : 'bg-[var(--bg-surface)]'}`} />
                        ...
                        <div className="space-y-3 pt-4 border-t border-[var(--border-subtle)]/50">
                            <div className="flex justify-between items-center">
                                <label className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider">Contactos en el Cliente</label>
                            ...
                            {(!Array.isArray(editForm.team?.client_contacts) || editForm.team!.client_contacts!.length === 0) && (
                                <p className="text-[var(--text-muted)] text-xs italic">No hay contactos adicionales registrados.</p>
                            )}

                            {Array.isArray(editForm.team?.client_contacts) && editForm.team.client_contacts.map((c, i) => (
                                <div key={i} className="bg-[var(--bg-base)] border border-[var(--border-subtle)] p-3 rounded-lg relative group">
                                    <button onClick={() => removeContact(i)} className="absolute top-2 right-2 text-[var(--text-muted)] hover:text-[var(--color-danger)]">
                                        <X className="w-4 h-4" />
                                    </button>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2 pr-6">
                                        <div className="flex items-center gap-2 border-b border-[var(--border-subtle)] pb-1">
                                            ...
                                            <input
                                                className="bg-transparent outline-none text-[var(--text-secondary)] text-sm w-full placeholder-[var(--text-muted)]"
                                                placeholder="Nombre"
                                                value={c.name}
                                        ...
                                        <div className="md:col-span-2 flex items-center gap-3 pt-1">
                                            <div className="flex items-center gap-2 bg-[var(--bg-base)] border border-[var(--border-subtle)] px-2 py-1 rounded">
                                                <input
                                                    type="checkbox"
                    ...
                    <div className="space-y-4">
                        <h3 className="text-sm font-bold text-[var(--color-primary)] uppercase border-b border-[var(--border-subtle)] pb-2">Estrategia y Próximos Pasos</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <Input
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider mb-2">Objetivo Principal</label>
                                <textarea
                                    className="w-full bg-[var(--bg-base)] border border-[var(--border-subtle)] rounded-lg p-3 text-[var(--text-secondary)] text-sm outline-none min-h-[80px]"
                            ...
                            <div>
                                <label className="block text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider mb-2">Problema Central</label>
                                <textarea
                                    className="w-full bg-[var(--bg-base)] border border-[var(--border-subtle)] rounded-lg p-3 text-[var(--text-secondary)] text-sm outline-none min-h-[80px]"
                    ...
                    <div className="space-y-4">
                        <div className="flex justify-between items-center border-b border-[var(--border-subtle)] pb-2">
                            <h3 className="text-sm font-bold text-[var(--color-primary)] uppercase">Roadmap (Hitos)</h3>
                        ...
                        {Array.isArray(editForm.milestones) && editForm.milestones.map((m, i) => (
                            <div key={i} className="flex gap-2 items-start bg-[var(--bg-base)] p-3 rounded-lg border border-[var(--border-subtle)]">
                                <div className="flex-grow grid grid-cols-1 md:grid-cols-3 gap-2">
                                    <input
                                        className="bg-transparent text-[var(--text-primary)] text-sm border-b border-[var(--border-subtle)] outline-none p-1"
                                        value={m.name}
                                    ...
                                    <input
                                        type="date"
                                        className="bg-transparent text-[var(--text-muted)] text-sm border-b border-[var(--border-subtle)] outline-none p-1"
                                        value={m.date}
                                        onChange={(e) => updateMilestone(i, 'date', e.target.value)}
                                    />
                                    <select
                                        className="bg-[var(--bg-base)] text-[var(--text-secondary)] text-sm border border-[var(--border-subtle)] rounded p-1 outline-none"
                                        value={m.status}
                                ...
                                <button onClick={() => removeMilestone(i)} className="text-[var(--color-danger)] hover:text-[var(--color-danger)] p-1">
                ...
                <div className="p-6 border-t border-[var(--border-subtle)] bg-[var(--bg-base)]/50 backdrop-blur-md rounded-b-xl flex justify-end gap-3 z-10">
